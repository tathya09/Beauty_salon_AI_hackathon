...
export function SalonCard({ salon, highlighted, compact, distanceLabel }: SalonCardProps) {
        ...
        <div className="relative overflow-hidden aspect-[4/3]">
          ...
          {distanceLabel && (
            <div className="absolute bottom-2 left-2">
              <span className="inline-flex items-center gap-1 bg-white/90 backdrop-blur-sm text-blue-700 text-xs font-semibold px-2 py-0.5 rounded-full shadow">
          ...
          {!compact &&
            salon.tags.slice(0, 3).map((tag) => (
              <Badge
                key={tag}
                variant="outline"
                className="text-xs mr-1 mt-1 text-rose-600 border-rose-200"
              >
                {tag}
              </Badge>
            ))}
